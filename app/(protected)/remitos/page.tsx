...
export default async function RemitosPage({
  ...
  // Rol para mostrar el acceso a calibración solo a admin/superadmin
  const {
    data: { user },
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {puedeCalibrar && (
            <Link
              href="/remitos/calibracion"
              title="Calibración de impresión"
              aria-label="Calibración de impresión"
              className="p-2 text-gray-500 border border-gray-300 rounded-md hover:bg-gray-50 hover:text-gray-700 transition-colors"
            >
              <svg
                className="w-5 h-5"
                fill="none"
