  ...
  return (
    <div className="space-y-5 max-w-4xl">
      ...
      <div className="flex items-center gap-2 text-sm text-gray-400">
        <Link href="/consultas" className="hover:text-gray-600 transition-colors">
          Consultas
        </Link>
        <span>/</span>
            ...
            <tbody>
              {historialCliente.map((h) => (
                <tr key={h.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50">
                  <td className="py-1.5">
                    <Link href={`/consultas/${h.id}`} className="font-bold text-indigo-600 hover:underline">
                      #{h.numero}
                    </Link>
                  </td>
                  ...
                  <td className="py-1.5">
                    <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${h.estado === "entregado" ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}>
                      {h.estado}
                    </span>
                  </td>
